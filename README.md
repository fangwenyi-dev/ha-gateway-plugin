...
v1.7.20 起，开窗器 cover 实体带完整位置能力面（`device_class: window` + `SET_POSITION` + 开度回显），HA 的 HomeKit 桥会将其映射为真正的 **Window（窗子）** accessory——Apple Home 里可拖动百分比开度并回显当前位置。此前实体缺少位置能力，HomeKit 只能按"开/关/停"三态呈现。

- **前提**：HA「设置 → 设备与服务 → HomeKit 桥」正常配桥接入 Apple Home；慧尖实体默认可见（桥按实体属性自动判定类型，无需额外配置）。
- **生效步骤**：升级加载项 → **重启 HA**（集成代码随加载项落盘、HA 启动时换载）→ 桥会以 Window 类型重建该 accessory。
- **位置语义**：校准过的电机上报真实开度（0-100），HomeKit 滑块与回显都是精确值；**未校准**电机（固件回 255）按"开=全开 / 关=全关"端点近似显示，电机完成一次校准后自动恢复精确。
