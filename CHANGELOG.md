...
- **活的宇宙背景层**（新增 `www/js/starsky.js`，mulberry32 固定种子＝
  全站同一片天）：三档氛围角云底 → 左下溢角星云一朵（禁内核雾，漂移 45s）
...
- **设备玻璃框只包自身内容**：`.device-list` 加 `align-items: start`——
  ...
  各框互不等高、间隙透出星空
- **星云 web 移植校准**：团体 68%→76%×86%、峰值 α.10→.16、blur 6→10px。
  小程序 α 定案语境是短内容手机页，web 满屏卡片会把星云核心整块遮住、
  边沿漏出仅 +5/765（实测不可见）；真页左下净增现 +63.7/765，入可见区间
  仍属克制（钉桩上限 .12→.18 并注理据）
