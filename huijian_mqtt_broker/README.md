...
[![版本](https://img.shields.io/badge/version-1.0.9-blue)]()
...
LoRa 网关 (MQTT 客户端)
    │
    │ MQTT TCP → HA_IP:1883 (Docker 端口映射)
    ▼
┌───────────────────────────────────────────────┐
...
└───────────────────────────────────────────────┘
    │
    │ HA MQTT 集成 (172.30.32.1:1883，自动配置)
    ▼
...
- ✅ 密码文件自动生成（使用预设用户名密码）
- ✅ HA MQTT 集成自动配置（连接到 172.30.32.1:1883）
...
端口 1883 被 HA 官方 Mosquitto broker 插件占用。请先停止 HA 官方的 Mosquitto broker 插件。
